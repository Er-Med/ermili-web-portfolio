...
Use the AskQuestion tool for quick structured questions where appropriate, but keep it conversational. If context is already available from the conversation (e.g. the user already shared the URL and described their product), skip those questions and confirm your understanding instead.
...
Always produce the audit as a markdown file in the project at `docs/audits/[site-name]-website-audit.md`. Create `docs/audits/` if it does not exist.
